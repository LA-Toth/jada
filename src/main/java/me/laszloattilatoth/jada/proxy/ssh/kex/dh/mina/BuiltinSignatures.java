 ...
 * Modifications of mina-sshd's org.apache.sshd.common.signature.BuiltinSignatures:
 * Copyright 2021 Laszlo Attila Toth
 *
 * Licensed under the Apache License, Version 2.0 (the "License")
 * The original code did not support the SSH algorithm names directly.
 *
 * Original copyright:
...
public enum BuiltinSignatures implements SignatureFactory {
    ...
    public static BuiltinSignatures fromAlgorithmName(String s) {
        if (GenericUtils.isEmpty(s)) {
            return null;
        }

        for (BuiltinSignatures c : VALUES) {
            if (s.equalsIgnoreCase(c.factoryName)) {
                return c;
            }
        }
